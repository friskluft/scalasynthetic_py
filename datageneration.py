...
class DatasetGenerator:
    ...
    def fill_tile (self, mask_mode, tile_x_index, tile_y_index, tile_x_size, tile_y_size, image_height, image_width, roi_height, roi_width, roi_base_data, padding, oversized_rois, num_oversized_diag_rois):
        ...
        for roi_tile_x in range(top_left_roi_tile[0],bottom_right_roi_tile[0]+1):
            for roi_tile_y in range(top_left_roi_tile[1],bottom_right_roi_tile[1]+1):
                ...
                # Intensity of mask tiles should reflect their label index 
                # while intensity tiles must have constant value across the image
                if mask_mode:
                    scalefactor = seg_id
                else:
                    scalefactor = 1
                ...
                if roi_tile_x == roi_tile_y and oversized_rois and roi_tile_x < num_oversized_diag_rois:
                    ...
                    tile_data[t_y_l_1:t_y_l_2+1, t_x_l_1:t_x_l_2+1] = scalefactor * roi_base_data_new[roi_top_y:roi_bottom_y+1, roi_left_x:roi_right_x+1]
                else:
                    tile_data[t_y_l_1:t_y_l_2+1, t_x_l_1:t_x_l_2+1 ] = scalefactor * roi_base_data[roi_top_y:roi_bottom_y+1,roi_left_x:roi_right_x+1]
    ...
    def generate_image_pair(self, n_rois, roi_size, padding, percent_oversized_diag_rois=0):
        ...
        with BioWriter(seg_file_name, max_workers=4, backend='python', X=image_width, Y=image_height, Z=1, C=1, T=1, dtype=np.uint32) as bw:
            ...
            for y in range(0,image_height, tile_y_size):
                ...
                for x in range(0, image_width, tile_x_size):
                    ...
                    tmp = self.fill_tile (
                        True,	# mask mode
                        tile_x_ind, tile_y_ind, tile_x_size, tile_y_size, 
                        image_height, image_width, 
                        roi_height, roi_width, 
                        self._roi_base_data, 
                        padding, 
                        True, 
                        num_oversized_diag_rois)
                    bw[y:y_max, x:x_max,0,0,0] = tmp[:y_max-y, :x_max-x]
                    tile_x_ind = tile_x_ind+1
                tile_y_ind = tile_y_ind+1

                # stayalive 
                print ("\t{0}%".format(round(y/image_height*100.0, 2)))
        ...
        print ("\n\tgenerating intensity {0} \n".format(int_file_name))
            ...
            for y in range(0,image_height, tile_y_size):
                ...
                for x in range(0, image_width, tile_x_size):
                    ...
                    tmp = self.fill_tile (
                        ...
                        tile_x_ind, tile_y_ind, tile_x_size, tile_y_size, 
                        image_height, image_width, 
                        sq_side, sq_side,	# height and width of the "stencil" image
                        cropped_image, 
                        padding, 
                        False, 
                        0)
